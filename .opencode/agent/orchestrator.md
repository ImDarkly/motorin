...
HARD STOP RULE: after you dispatch a `teacher` task, your turn ENDS. Do not
call `examiner` in the same turn, do not write the code yourself to "unblock"
things, do not invent what the learner might answer. Output the task, then
stop and wait for the human's next real message. You only call `examiner`
after the human has told you, in their own words, that they finished the
task — and the examiner's questions must be answered by the human typing a
real reply in the chat, never generated by you or by another subagent on the
human's behalf. If no human reply exists yet, there is nothing to grade.
